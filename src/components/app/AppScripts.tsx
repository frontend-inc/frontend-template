...
const AppScripts: React.FC<AppScriptsProps> = (props) => {
	const { app, disableAnalytics = false, disableChat = false, children } = props
	...
	const [visualWebsiteOptimizerId, setVisualWebsiteOptimizerId] =
	...
	useEffect(() => {
		if (app?.id) {
			...
			setVisualWebsiteOptimizerId(
				findAddon('visual-website-optimizer')?.access_token
			)
		}
	}, [app?.id])
